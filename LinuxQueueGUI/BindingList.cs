    ...
    protected override int FindCore(PropertyDescriptor prop, object key) {
        ...
        if (key != null) {
            ...
            for (int i = 0; i < Count; ++i) {
                item = (T)Items[i];
                if (propInfo.GetValue(item, null) != null && propInfo.GetValue(item, null).Equals(key))
                    return i;
            }
        }
        return -1;
...
public static class Extension {
    public static FilteredBindingList<T> ToFilteredBindingList<T>(this IList<T> list) {
        var fList = new FilteredBindingList<T>();
    ...
    public static BindingSource ToBindingSource<T>(this IList<T> list) {
        var fList = list.ToFilteredBindingList();

        return new BindingSource(fList, null);
    }
}
