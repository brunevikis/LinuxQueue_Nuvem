...
then
    ...
    then
        ( { cd "$WD" && $CMD                             > $DIR/run_log/$commandname.run 2>&1; echo "EXITCODE=$?" >> $DIR/$rcommand; cd $DIR; sleep 2; echo "ETIME=$(date +%Y-%m-%d\ %T)" >> $rcommand; rm $rcommand $fcommand;  } & { echo "PID=$!" >> $rcommand; echo "STIME=$(date +%Y-%m-%d\ %T)" >> $rcommand; echo "$(date +%Y-%m-%d\ %T) : PID=$!"; } );
    elif [[ "$CLUSTER" == "null" ]]
    then
